...
double simulate_recursion(double t, double x_1, double x_2, double x_3, double lambda) {
    ...
    if (tau < t) {
        ...
        double chi_1 = simulate_recursion(t - tau, x_1 + tau * sin(alpha) * cos(theta), x_2 + tau * sin(alpha) * sin(theta), x_3 + tau * cos(alpha), lambda);
        double chi_2 = simulate_recursion(t - tau, x_1 + tau * sin(alpha) * cos(theta), x_2 + tau * sin(alpha) * sin(theta), x_3 + tau * cos(alpha), lambda);
        return (tau / lambda) * exp(lambda * tau) * chi_1 * chi_2;
    } else {
        double theta  = angle_distribution(generator);
        double p = uniform_distribution(generator);
        double alpha = acos(1-2*p);
        return exp(lambda * t) * (6 / pow(x_1 + t * sin(alpha) * cos(theta) + x_2 + t * sin(alpha) * sin(theta) + x_3 + t * cos(alpha), 2) + (t * sin(alpha) * cos(theta) + t * sin(alpha) * sin(theta) + t * cos(alpha)) * (-12 / pow(x_1 + t * sin(alpha) * cos(theta) + x_2 + t * sin(alpha) * sin(theta) + x_3 + t * cos(alpha), 3)) + t * (-24) / pow(x_1 + t * sin(alpha) * cos(theta) + x_2 + t * sin(alpha) * sin(theta) + x_3 + t * cos(alpha), 3));
    }
}

void run_simulation(int start, int end, double t, double x_1, double x_2, double x_3, double lambda, xt::xarray<double>& results) {
    for (int i = start; i < end; ++i) {
        results[i] = simulate_recursion(t, x_1, x_2, x_3, lambda);
    }
}

double simulate(double t, double x_1, double x_2, double x_3, double lambda, int total_sims) {
    ...
    for (int i = 0; i < NUM_THREADS; ++i) {
        ...
        threads.emplace_back(run_simulation, start, end, t, x_1, x_2, x_3, lambda, std::ref(results));
    }
...
int main()
{
    string directoryPath = "../Simulation_08/output";
    if (!std::filesystem::exists(directoryPath)) {
        std::filesystem::create_directories(directoryPath);
    }

    // Create output directory if not exists for the Monte Carlo results
    string resultsDirectory = "../Simulation_08/results";
    ...
    freopen("../Simulation_08/output/my_output.out", "w", stdout);
    // Open log file for stderr (error messages)
    freopen("../Simulation_08/output/my_error.err", "w", stderr);
    ...
    try {
        ...
        string file_name = "../Simulation_08/results/monte_carlo.csv"; // Monte Carlo results file

        for (int k = 0; k < num_estimations; k++) {
            double t = static_cast<double>(k);
            t /= 10.;
            auto start_time = std::chrono::high_resolution_clock::now();
            double estimated_value = simulate(t, x_1, x_2, x_3, lambda, NUM_SIMULATIONS);
            auto end_time = std::chrono::high_resolution_clock::now();
            std::chrono::duration<double> elapsed_time = end_time - start_time;
            cout << "x_1 = " << x_1 << ", x_2 = " << x_2 << ", x_3 = " << x_3 << ", t = " << t << ", estimated_value = " 
                << estimated_value << ", Execution time: " << elapsed_time.count() << " seconds." << endl;
            ...
            cout << "Updated CSV with x_1 = " << x_1 << ", x_2 = " << x_2 << ", x_3 = " << x_3 << ", t = " << t 
                << ", estimated_value = " << estimated_value << endl;
            cout.flush();  // Ensure that output is written immediately to the file
        }
    }
