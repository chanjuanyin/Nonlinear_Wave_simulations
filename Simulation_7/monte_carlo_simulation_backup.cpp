...
complex<double> simulate_recursion(double t, double x, double lambda) {
    ...
    if (tau < t) {
        ...
        if (J < 1./2.) {
            ...
            complex<double> v = simulate_recursion(t - tau, x + y, lambda);
            return - 2. * tau / lambda * exp(lambda * tau) * v;
        } else {
            std::uniform_real_distribution<> time_distribution(-tau, tau);
            double y = time_distribution(generator);
            complex<double> v1 = simulate_recursion(t - tau, x + y, lambda);
            complex<double> v2 = simulate_recursion(t - tau, x + y, lambda);
            complex<double> v3 = simulate_recursion(t - tau, x + y, lambda);
