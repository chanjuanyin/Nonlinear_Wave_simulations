...
complex<double> phi(complex<double> z_1, complex<double> z_2) {
    return tanh( (complex<double>(0., 1.)/(2*sqrt(3))) * (z_1+z_2) );
}

complex<double> partial_z_1_phi(complex<double> z_1, complex<double> z_2) {
    return (complex<double>(0., 1.)/(2*sqrt(3))) * pow(sech( (complex<double>(0., 1.)/(2*sqrt(3))) * (z_1+z_2) ), 2);
}
...
complex<double> psi(complex<double> z_1, complex<double> z_2) {
    return -sqrt(2./3.) * pow(sech( (complex<double>(0., 1.)/(2*sqrt(3))) * (z_1+z_2) ), 2);
}
...
SimulationStats simulate(complex<double> z_1, complex<double> z_2, double t, complex<double> c, double lambda, int total_sims) {
...
int main()
{
    ...
    double lambda = 0.3495487; // Choose lambda = t_0, please refer to the Plot_history.ipynb Jupyter Notebook for details of how to calculate maximum t_0 (existence time of classical solution)
    ...
    string mean_file_name = "../Simulation_06/results/monte_carlo_mean.csv";
    string var_file_name = "../Simulation_06/results/monte_carlo_variance.csv";
    string runtime_file_name = "../Simulation_06/results/monte_carlo_runtime.csv";

    for (int k = 0; k < num_estimations; k++) {
        double t = static_cast<double>(k);
        t /= 10.;
        auto start_time = std::chrono::high_resolution_clock::now();
        SimulationStats estimated_value = simulate(z_1, z_2, t, c, lambda, NUM_SIMULATIONS);
