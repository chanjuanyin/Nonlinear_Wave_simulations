...
double simulate_recursion(double t, double x_1, double x_2, double lambda) {
    ...
    if (tau < t) {
        ...
        double v1 = simulate_recursion(t - tau, x_1 + R * cos(theta), x_2 + R * sin(theta), lambda);
        double v2 = simulate_recursion(t - tau, x_1 + R * cos(theta), x_2 + R * sin(theta), lambda);
...
int main()
{
    ...
    try {
        ...
        string file_name = "../Simulation_04/results/monte_carlo.csv"; // Monte Carlo results file
