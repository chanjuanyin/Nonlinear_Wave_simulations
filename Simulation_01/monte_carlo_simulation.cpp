...
double simulate_recursion(double t, double x, double lambda) {
    ...
    if (tau < t) {
        ...
        return (tau / lambda) * exp(lambda * tau) * v1 * v2;
    } else { // tau >= t
        double Y = t * uniform_distribution(generator);
        return exp(lambda * t) * ( (6. / 2.) * pow(x + t, -2) + (6. / 2.) * pow(x - t, -2) + t * (-12. * sqrt(2)) * pow(x + Y, -3));
    }
}
...
int main()
{
    string directoryPath = "../Simulation_01/output";
    if (!std::filesystem::exists(directoryPath)) {
        std::filesystem::create_directories(directoryPath);
    }

    // Create output directory if not exists for the Monte Carlo results
    string resultsDirectory = "../Simulation_01/results";
    ...
    freopen("../Simulation_01/output/my_output.out", "w", stdout);
    // Open log file for stderr (error messages)
    freopen("../Simulation_01/output/my_error.err", "w", stderr);
    ...
    try {
        ...
        string file_name = "../Simulation_01/results/monte_carlo.csv"; // Monte Carlo results file
