...
SimulationStats simulate(complex<double> z_1, complex<double> z_2, complex<double> z_3, double t, complex<double> c, double lambda, int total_sims) {
...
int main()
{
    ...
    string mean_file_name = "../Simulation_09/results/monte_carlo_mean.csv";
    string var_file_name = "../Simulation_09/results/monte_carlo_variance.csv";
    string runtime_file_name = "../Simulation_09/results/monte_carlo_runtime.csv";

    for (int k = 0; k < num_estimations; k++) {
        double t = static_cast<double>(k);
        t /= 10.;
        auto start_time = std::chrono::high_resolution_clock::now();
        SimulationStats estimated_value = simulate(z_1, z_2, z_3, t, c, lambda, NUM_SIMULATIONS);
        auto end_time = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed_time = end_time - start_time;
        cout << "z_1 = " << z_1.real() << " + " << z_1.imag() << " * i, z_2 = " << z_2.real() 
            << " + " << z_2.imag() << " * i, z_3 = " << z_3.real() << " + " << z_3.imag() 
            << " * i, t = " << t << ", estimated_value u(z,t) = " << estimated_value.mean_real 
            << " + " << estimated_value.mean_imag << " * i, Execution time: " << elapsed_time.count() 
            << " seconds."<< endl;
