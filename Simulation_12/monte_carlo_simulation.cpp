...
int main()
{
    ...
    xt::xarray<double> arr_real = xt::zeros<double>({1, num_estimations + 1});
    xt::xarray<double> arr_imag = xt::zeros<double>({1, num_estimations + 1});
    string file_name_real = "../Simulation_12/results/monte_carlo_real.csv";
    string file_name_imag = "../Simulation_12/results/monte_carlo_imag.csv";

    for (int k = 0; k <= num_estimations; k++) {
        double k_double = static_cast<double>(k);
        complex<double> z = z_start + (z_end - z_start) / num_estimations_double * k_double;

        auto start_time = std::chrono::high_resolution_clock::now();
        std::pair<double, double> estimated_value = simulate(z, t, c, lambda, NUM_SIMULATIONS);
        auto end_time = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsed_time = end_time - start_time;
        std::cout << "z = " << z.real() << " + " << z.imag() << " * i, t = " 
            << t << ", estimated_value u(z, t) = " << estimated_value.first 
            << " + " << estimated_value.second << " * i, Execution time: " 
            << elapsed_time.count() << " seconds."<< std::endl;

        // Update the value in the array and write to CSV
        xt::view(arr_real, 0, k) = estimated_value.first;
        xt::view(arr_imag, 0, k) = estimated_value.second;
